...
public class CrescentSearchRequestWrapper {
	...
	public Query getFilterQuery() throws CrescentInvalidRequestException {
		...
		if(filterQueryString != null && filterQueryString.length() > 0) {
			...
			try {
				...
				return queryParser.getQuery(getIndexedFields(), filterQueryString, collection.getSearchModeAnalyzer(), searchRequest.getRegexQuery());

			} catch (Exception e) {
				logger.error("Error In getFilterQuery ", e);
				throw new CrescentInvalidRequestException(e.getMessage());
			}
